...
int mqtt_disconnect(mqtt_broker_handle_t* broker) {
	...
	// Send the packet
	if(broker->send(&ssl, packet, sizeof(packet)) < sizeof(packet)) {
		return -1;
	}
...
int mqtt_publish(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint16 msglen, uint8 retain) {
	return mqtt_publish_with_qos(broker, topic, msg, msglen, retain, QOS_VALUE2, NULL);
}

int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint16 msgl, uint8 retain, uint8 qos, uint16* message_id) {
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16* message_id) {
	uint16 topiclen = strlen(topic);
	uint8 utftopicSize = 0;
	uint8 *utf_topic = (uint8 *)malloc(topiclen+3);// Topic size (2 bytes), utf-encoded topic, QoS byte
	// Variable header
	uint8 *var_header = NULL; // Topic size (2 bytes), utf-encoded topic
	uint8 *fixed_header = NULL;
	uint8 fixedHeaderSize = 0,var_headerSize = 0;    // Default size = one byte Message Type + one byte Remaining Length
...
int mqtt_unsubscribe(mqtt_broker_handle_t* broker, const char* topic, uint16* message_id) {
	uint16 topiclen = strlen(topic);
	uint8 *utf_topic = (uint8 *)malloc(topiclen+2);// Topic size (2 bytes), utf-encoded topic	
	// Variable header
	uint8 var_header[2]; // Message ID
	uint8 fixedheadersize = 1+sizeof(var_header)+(topiclen+2);
	uint8 *fixed_header = (uint8 *)malloc(fixedheadersize);
	uint8 packetsize = sizeof(var_header)+fixedheadersize+(topiclen+2);
	uint8 *packet = (uint8 *)malloc(packetsize);
	...
	*(utf_topic+1) = topiclen&0xFF;
	...
	free(fixed_header);
	// Send the packet
	if(broker->send(&ssl, packet, packetsize) < packetsize) {
		free(packet);
		return -1;
	}
	free(packet);
